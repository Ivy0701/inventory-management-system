...
系统已自动创建预设的销售员和仓库管理员账号，这些账号在服务器启动时自动初始化（如果不存在）。
...
| 账号 | 密码 | 姓名 | 角色 |
|------|------|------|------|
| warehouse001 | 123456 | 仓库管理员1 | warehouse |
| warehouse002 | 123456 | 仓库管理员2 | warehouse |
...
1. 在首页选择"仓库管理员"
2. 点击"登录"按钮
3. 输入账号：`warehouse001` 或 `warehouse002`
4. 输入密码：`123456`
5. 输入验证码（不区分大小写）
6. 点击"登录"
...
A: 确保：
1. 选择了正确的角色（销售员或仓库管理员）
2. 输入的是正确的预设账号（sales001/sales002 或 warehouse001/warehouse002）
3. 账号的角色与选择的角色一致
